    ...
    C: Sx126xVariant,
{
    ...
    async fn implicit_header_mode_handling(&mut self) -> Result<(), RadioError> {
    ...
    C: Sx126xVariant,
{
    ...
    async fn process_irq_event(
        ...
        let irq_state = self.get_irq_status(radio_mode, cad_activity_detected).await;
        ...
        if let (RadioMode::Receive(RxMode::Single(_)), Ok(Some(IrqState::Done))) = (radio_mode, &irq_state) {
            // implicit header mode timeout behavior (see DS_SX1261-2_V1.2 datasheet chapter 15.3)
            self.implicit_header_mode_handling().await?;
        }
